...
#### 3.1.1 符号说明
斜体英文字母表示标量（x,d），加粗斜体小写表示向量（x），加粗斜体大写表示矩阵（X），希腊字母为超参数（λ）。令 Σ 为股票集合，N=|Σ|。记 c_u[t] 为股票 u 在 t 的收盘价，x_u[t]∈R^F 为该时刻的特征向量。记动态图 G_t=(V,E_t,W_t)。

#### 3.1.2 问题定义
目标为未来 d 日回报预测：
```text
y_u[t] = (c_u[t+d] - c_u[t]) / c_u[t],  u∈Σ,  t∈[1,L]
```
给定全体股票历史特征与图结构 X，学习映射 f(·;Θ) 输出点预测与不确定性：
```text
{ y_hat_u }_{u∈Σ} = f(X; Θ)
```

### 3.2 整体结构与模块功能（总览）
数据经预处理后分两路进入深度学习预测部分：（1）股票内路径用 SAT-FAN 自适应提取多尺度时频模式；（2）股票间路径用 NGC 生成动态图并驱动 RGAT 学习时变传导；随后经“跨视角融合模块”耦合内外部信息并输出 μ、分位数与不确定性；并行地，市场模块（HMM+Probe+MILAN）产生宏观/冲击成本与回报分解（可作为状态与成本）；最终，两路状态晚融合进入强化学习，策略输出动作，动作映射成交易意图再反馈给市场模块评估冲击，形成闭环（见 assets/fda_architecture_ltr.svg）。
...
（举例）如某标的在 2 个月区间内“放量上涨—高位震荡—缩量回调”三段模式交替。成交量与收益的联动、短期波动与中期节律共现，单一尺度难以同时覆盖。

（方法与公式，沿数据流）
1) 时频特征增强。对输入 x_u[1:L] 做时间标准化与频域扩展：
```text
tilde_x_u[t] = (x_u[t] - mu_u) / (sigma_u + eps)
f_u = TopK( | rFFT( x_u[channel] ) | )
```
将 f_u 按时间广播并与 tilde_x_u[t] 拼接得 bar_x_u[t]。

2) 多尺度 TCN 专家。将 bar_x_u[1:L] 分别送入短/中/长三路 Dilated TCN：
```text
H^(s) = TCN_s( bar_x_u[1:L] ),  s ∈ {short, mid, long}
```
其中每层空洞卷积的膨胀率依次为 1,2,4,…，感受野 R_s 递增，覆盖不同时间跨度。

3) 专家内时间注意力。对每路输出 H^(s)=[h_s[1],…,h_s[L]]：
```text
...
4) 专家间融合注意力。以 [c_short, c_mid, c_long] 为输入，做自注意/多头注意融合：
```text
H_intra = Attn_fuse( [c_short, c_mid, c_long] )
```

#### 3.3.2 股票间关系模块（NGC→RGAT）
（为何）股票间存在行业/供给链/风格/资金等传导，且呈非线性与时变性。静态相关或固定邻接无法反映“当下”的非对称影响与因果方向。

（举例）两只同业股在某段时间高度联动，但政策扰动或资金风格切换会改变领先—滞后关系，单一静态图会误导信息聚合方向。

（方法与公式，沿数据流）
1) NGC 因果识别。对目标股票 j 构造滞后设计矩阵（全体股票过去 L_lag 个收益）：X_lag∈R^{T×(N·L_lag)}，以 MLP 预测 y^(j)∈R^T：
```text
min_{Θ_MLP}  MSE( MLP(X_lag; Θ_MLP), y^(j) )
             + λ_grp · Σ_{i=1..N} || W[:, G_i] ||_F
```
其中 G_i 为来源股票 i 的“跨滞后”列组，组范数促使整支股票被整体选择/剔除，得到因果强度 A_ij。

2) 道德化与稀疏图。A 经道德化得到无向 M，再转为 edge_index/edge_weight（并给出 adj_indices）。

3) RGAT 聚合。对节点特征 x_i 与边集 E：
```text
h_i^(h) = W^(h) · x_i
...
```
预测头输出 mu、分位数 q10/q50/q90 与不确定性 u。

### 3.4 强化学习部分（为何→举例→方法与公式）
（为何）在容量受限与冲击显著的场景，单纯最大化预测收益容易导致“高换手—高冲击—高成本”。策略需在“收益—成本—风险”间权衡。

（举例）在流动性脆弱期（rv 高、成交活跃异常 z 高），同等幅度的调仓会产生更高滑点与永久冲击；策略若不感知成本，将在实盘中显著劣化。

（方法与公式）
1) 状态（晚融合）：
```text
s_t = vec( rl_state_t  ||  z_macro, risk, r_norm, r_impact, c )
# rl_state_t 包含: mu, q, u, h_intra, h_inter, styles
```

2) 动作与环境。令动作为目标权重 w_t（或离散选择），交易向量 Δw_t=w_t−w_{t-1}。意图向量：
```text
intent_t = sign(Δw_t) ⊙ ( |Δw_t| / ADV_t )
```

3) 奖励（影响感知）：
```text
R_t = (w_{t-1} · r_t)
      - Σ_i [ κ_i,t · σ_i,t · ( |Δw_i,t| / ADV_i,t )^{α_i,t}  +  β_i,t · |Δw_i,t| ]
```

4) DQN 更新。以 Q 网络 Q_θ 学习：
```text
y_t = R_t + γ · max_a Q_θ( s_{t+1}, a )
L_DQN = ( y_t - Q_θ( s_t, a_t ) )^2
```
解释性：对 Q_θ(s_t, a_t) 以 SHAP 归因，输出正/负 Top-K 驱动特征（决策备忘录）。

### 3.5 全模型的损失/奖励函数（联合目标）
监督与正则：
```text
L_pred = MSE( mu, r_{t+1} )  +  Σ_{q∈{0.1,0.5,0.9}} Pinball(q)
L_mkt  = MSE( r_total, r )  +  α · MSE( c, c_proxy )
R_group = Σ_i || W[:, G_i] ||_F
R_style = || W_style · W_style^T - I ||_F^2  +  λ_sp · ||E||_1
```
强化学习：L_DQN 如上，reward 为 R_t。
...
### 3.6 小结
本章给出了从符号与问题定义开始，沿数据流对股票内（SAT-FAN）、股票间（NGC→RGAT）与融合模块的完整推导，并阐明了为何与如何引入强化学习及其状态/环境/动作/奖励的表述。联合目标在“预测精度—成本约束—可解释性”之间建立可优化的权衡，为容量敏感的交易提供系统化、可审计的学习框架。
